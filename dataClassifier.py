    ...
    for x in range(1, DIGIT_DATUM_WIDTH):
        for y in range(1, DIGIT_DATUM_HEIGHT):
            if datum.getPixel(x, y) > datum.getPixel(x, y - 1) :
                features[("verti", x, y)] = 1
            else :
                features[("verti", x, y)] = 0
            if datum.getPixel(x, y) > datum.getPixel(x - 1, y) :
                features[("horiz", x, y)] = 1
            else :
                features[("horiz", x, y)] = 0

    visited = set()
    whiteSpace = 0
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            if datum.getPixel(x, y) < 2 and (x, y) not in visited :
                whiteSpace += 1
                stack = [(x, y)]
                while stack:
                    point = stack.pop()
                    visited.add(point)
                    # prepare neighbors array
                    neighbors = []
                    if point[0] > 0:
                        neighbors.append((point[0] - 1, point[1]))
                    if point[0] < DIGIT_DATUM_WIDTH - 1:
                        neighbors.append((point[0] + 1, point[1]))
                    if point[1] > 0:
                        neighbors.append((point[0], point[1] - 1))
                    if point[1] < DIGIT_DATUM_HEIGHT - 1:
                        neighbors.append((point[0], point[1] + 1))
                    for neighbor in neighbors :
                        if datum.getPixel(*neighbor) < 2 and neighbor not in visited:
                            stack.append(neighbor)

    # print "contiguous : ----------------------------------------------------------" + str(contiguous)
    features["0"] = whiteSpace % 2
    features["1"] = (whiteSpace >> 1) % 2

    # for i in features :
    #     print "i: " + str(i) + " value: " + str(features[i])
...
def enhancedPacmanFeatures(state, action):
    ...
    distance = list()
    for index in state.generateSuccessor(0, action).getGhostPositions() :
        distance.append(util.manhattanDistance(
            state.generateSuccessor(0, action).getPacmanPosition(), index))
        ghosts = sorted(distance)

    for index in xrange(min(len(ghosts), 1)):
        features[("ghost", index)] = 5 / (0.1 + ghosts[index])
    # distance = list()
    # for i in capsules :
    #     distance.append(util.manhattanDistance(pacmanPosition, i))
    #     nearest_caps = sorted(distance)
    capsulates = sorted([util.manhattanDistance(
        state.generateSuccessor(0, action).getPacmanPosition(), index)
                         for index in state.generateSuccessor(0, action).getCapsules()])

    food = []
    for x, row in enumerate(state.getFood()) :
        for y, hasFood in enumerate(row) :
            if hasFood :
                food.append((x, y))
    distance = list()
    for index in food :
        distance.append(util.manhattanDistance(
            state.generateSuccessor(0, action).getPacmanPosition(), index))
        food_near = sorted(distance)

    features["nearest_ghost"] = ghosts[0] * 1.0
    for index in xrange(min(len(capsulates), 1)):
        features[("capsule", index)] = 15 / (1 + capsulates[index])
    for index, weight in zip(xrange(min(len(food_near), 5)), [1.3, 0.8] + [0.9] * 3):
        features[("food", index)] = weight * food_near[index]

    features["win"] = state.isWin()
    features["lose"] = state.isLose()
    features["score"] = state.getScore() * 10
    features["capsule count"] = len(state.generateSuccessor(0, action).getCapsules()) * 10
